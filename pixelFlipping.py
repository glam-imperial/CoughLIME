...
def evaluate_data(number_components):
    ...
    _ = next(csv_reader)
    number_files = 0
    true_morf = [0] * (number_components - 1)
    true_rand = [0] * (number_components - 1)

    for row in csv_reader:
        if row[2] == 'morf':
            prediction_whole = np.rint(float(row[1]))
            for i in range(number_components - 1):
    ...
    for removed in range(number_components - 1):
        path_save_summary = f"./eval/{removed + 1}_removed_components.txt"
...
def main_pixel_flipping(number_components, factorization_type, results_path, data_directory, num_samples):
...
if __name__ == '__main__':
    # main_pixel_flipping(7, 'temporal', './eval/', '/Users/anne/Documents/Uni/Robotics/Masterarbeit/MA_Code/DICOVA/DiCOVA_Train_Val_Data_Release/AUDIO/', 200) # TODO: adapt path
    evaluate_data(7)
